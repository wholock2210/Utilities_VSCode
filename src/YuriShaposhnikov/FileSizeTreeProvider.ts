...
import {RegexFilterRule} from './FilterRule';
import {FileFilterRule} from './FilterRule';
import {ExtensionFilterRule} from './FilterRule';
import {FolderFilterRule}  from './FilterRule';
...
export class FileSizeTreeDataProvider implements vscode.TreeDataProvider<any> {
    ...
    async getRoot(): Promise<any[]> {
		...
		if (this.handleGitignoreConfig && !this.gitignore) {
			...
			if (gitIgnore !== null) {
				this.gitignoreRoot = vscode.Uri.joinPath(vscode.workspace.workspaceFolders![0].uri, '.gitignore').path.length - 10;
