...
export class GitHubFileSystemProvider implements FileSystemProvider {
	...
	async fetchRepoOf(uri: vscode.Uri) {
        ...
        if (!meta) {
			const repoResponse = await laxios.get<RepoResponse>(`/repos/${owner}/${name}`);
			let branch = repoResponse.data.default_branch;
			this.defaultBranches[`${owner}/${name}`] = branch;
		
			const refResponse = await laxios.get<GitRefResponse>(`/repos/${owner}/${name}/git/refs/heads/${branch}`);
			oid = refResponse.data.object.sha;
		
		} else if (meta.ref?.type === 2) {
			oid = meta.ref.id;
		
		} else if (meta.ref?.type === 3) {
			const prResponse = await laxios.get<PullRequestResponse>(`/repos/${owner}/${name}/pulls/${meta.ref.id}`);
			oid = prResponse.data.head.sha;
		
		} else {
			const refResponse = await laxios.get<GitRefResponse>(`/repos/${owner}/${name}/git/ref/heads/${meta.ref.id}`, {
				validateStatus: () => true
			});
			oid = refResponse?.data?.object?.sha;
		
			if (!oid) {
				const tagResponse = await laxios.get<GitRefResponse>(`/repos/${owner}/${name}/git/ref/tags/${meta.ref.id}`);
				oid = tagResponse?.data?.object?.sha;
			}
		}
		
		const treeResponse = await laxios.get<GitTreeResponse>(`/repos/${owner}/${name}/git/trees/${oid}?recursive=1`);
		const trees = treeResponse.data.tree;
